#!/usr/bin/env python3
"""
High PF + High Frequency: Combine selectivity with more trades.
Approaches:
A) Multiple concurrent positions (1-5)
B) Hybrid: primary (PF=7) + secondary relaxed entries
C) Intraday re-entry after TP/SL
"""
...
# ============================================================
def backtest_multipos(data, config):
    ...
    max_pos = config.get('max_pos', 1)
    reentry=config.get('reentry', False)

    if hour is not None and config.get('session_filter','all') == 'no_asian':
        session_ok = hour > 7
    else:
    ...
    eq = INITIAL_EQUITY; curve = np.zeros(n); pnls = []
    positions = []  # list of (direction, entry_price, sl, tp, size)
    last_exit_bar = -10
    ...
    for i in range(n):
        ...
        for pos in positions:
            dr, ep, sl_val, tp_val, sz = pos
            exited = False
            if dr == 1:  # Long
                if low[i] <= sl_val:
                    pnl=(sl_val-ep)*sz*100-SPREAD*sz*100; eq+=pnl; pnls.append(pnl); exited=True
                elif high[i] >= tp_val:
                    pnl=(tp_val-ep)*sz*100-SPREAD*sz*100; eq+=pnl; pnls.append(pnl); exited=True
            else:  # Short
                if high[i] >= sl_val:
                    pnl=(ep-sl_val)*sz*100-SPREAD*sz*100; eq+=pnl; pnls.append(pnl); exited=True
                elif low[i] <= tp_val:
                    pnl=(ep-tp_val)*sz*100-SPREAD*sz*100; eq+=pnl; pnls.append(pnl); exited=True
            if not exited:
        ...
        if len(positions) < max_pos:
            can_entry = (reentry) or (i - last_exit_bar > 0)
            if can_entry:
                signal = 0
                ...
                if signal != 0:
                    ...
                    positions.append((signal, ep, sl_val, tp_val, sz))
                    last_exit_bar = i

        curve[i] = eq
    return curve, np.array(pnls), len(pnls)
...
print("\n" + "="*70)
print("HIGH PF + HIGH FREQUENCY EXPERIMENTS")
print("="*70)

experiments = {
    # A) Multi-position
    'A1_base_pf7': {**base_pf7, 'max_pos':1, 'reentry':False},
    'A2_multipos_2': {**base_pf7, 'max_pos':2, 'reentry':False},
    'A3_multipos_3': {**base_pf7, 'max_pos':3, 'reentry':False},
    'A4_multipos_5': {**base_pf7, 'max_pos':5, 'reentry':False},
    'A5_multipos_2_re': {**base_pf7, 'max_pos':2, 'reentry':True},
    'A6_multipos_3_re': {**base_pf7, 'max_pos':3, 'reentry':True},
    'A7_multipos_5_re': {**base_pf7, 'max_pos':5, 'reentry':True},

    # B) Relaxed params with multi-position
    'B1_rt65_mp2': {**base_pf7, 'rt':0.65, 'max_pos':2, 'reentry':True},
    'B2_rt65_mp3': {**base_pf7, 'rt':0.65, 'max_pos':3, 'reentry':True},
    'B3_rt65_mp3_ct10': {**base_pf7, 'rt':0.65, 'ct':0.10, 'max_pos':3, 'reentry':True},
    'B4_rt68_mp3_sl10': {**base_pf7, 'rt':0.68, 'sl':1.0, 'max_pos':3, 'reentry':True},
    'B5_rt68_mp5_sl10': {**base_pf7, 'rt':0.68, 'sl':1.0, 'max_pos':5, 'reentry':True},

    # C) Tighter SL for faster trades
    'C1_sl10_mp3': {**base_pf7, 'sl':1.0, 'tp':2.5, 'max_pos':3, 'reentry':True},
    'C2_sl10_mp5': {**base_pf7, 'sl':1.0, 'tp':2.5, 'max_pos':5, 'reentry':True},
    'C3_sl12_mp3_tp25': {**base_pf7, 'sl':1.2, 'tp':2.5, 'max_pos':3, 'reentry':True},
    'C4_sl12_mp5_tp25': {**base_pf7, 'sl':1.2, 'tp':2.5, 'max_pos':5, 'reentry':True},

    # D) Lower TT for more MR trades
    'D1_tt45_mp3': {**base_pf7, 'tt':0.45, 'max_pos':3, 'reentry':True},
    'D2_tt45_mp5': {**base_pf7, 'tt':0.45, 'max_pos':5, 'reentry':True},
    'D3_tt45_sl10_mp5': {**base_pf7, 'tt':0.45, 'sl':1.0, 'tp':2.5, 'max_pos':5, 'reentry':True},

    # E) Lower risk per trade for more positions
    'E1_risk1_mp5': {**base_pf7, 'risk':0.005, 'max_pos':5, 'reentry':True},
    'E2_risk1_mp10': {**base_pf7, 'risk':0.005, 'max_pos':10, 'reentry':True},
    'E3_risk05_mp5_sl10': {**base_pf7, 'risk':0.005, 'sl':1.0, 'tp':2.5, 'max_pos':5, 'reentry':True},
    'E4_risk08_mp5_sl12': {**base_pf7, 'risk':0.008, 'sl':1.2, 'tp':2.5, 'max_pos':5, 'reentry':True},
}

print(f"\n{'#':<4} {'Config':<25} {'IS PF':>6} {'T/D':>5} {'OOS PF':>6} {'WR':>5} {'DD%':>5} {'T/D':>5} {'N':>5} {'Ret%':>8}")
print("─"*85)

results = []
for name, cfg in experiments.items():
    try:
        eq_is, pnl_is, n_is = backtest_multipos(is_data, cfg)
        eq_oos, pnl_oos, n_oos = backtest_multipos(oos_data, cfg)
        ...
        print(f"{len(results):<4} {name:<25} {m_is['pf']:>6.2f} {m_is['tpd']:>5.1f} "
              f"{m_oos['pf']:>6.2f} {m_oos['wr']:>4.0%} {m_oos['mdd']:>4.1f} "
              f"{m_oos['tpd']:>5.1f} {m_oos['n']:>5} {m_oos['ret']:>+7.1f}%{in_range}")
    except Exception as e:
        print(f"  Error {name}: {e}")

# Top configs in range
print("\n" + "="*70)
print("TOP CONFIGS IN RANGE (2-10 trades/day)")
print("="*70)
...
for i, r in enumerate(in_range[:10]):
    oos = r['oos']
    print(f"  {i+1}. {r['name']:<25} PF={oos['pf']:.2f} WR={oos['wr']:.0%} DD={oos['mdd']:.1f}% "
          f"T/D={oos['tpd']:.1f} N={oos['n']} Ret={oos['ret']:+.1f}%")

# Best in range
if in_range:
    best = in_range[0]
    with open('best_highpf_freq.json', 'w') as f:
        json.dump(best, f, indent=2, default=str)
    print(f"\nSaved best_highpf_freq.json")

# Also show highest PF regardless of frequency
print("\n" + "="*70)
print("HIGHEST PF (regardless of frequency)")
print("="*70)
by_pf = sorted(results, key=lambda x: x['oos']['pf'], reverse=True)
for i, r in enumerate(by_pf[:5]):
    oos = r['oos']
    print(f"  {i+1}. {r['name']:<25} PF={oos['pf']:.2f} T/D={oos['tpd']:.1f} WR={oos['wr']:.0%} DD={oos['mdd']:.1f}%")

print("\n✅ High PF + frequency experiments complete!")
