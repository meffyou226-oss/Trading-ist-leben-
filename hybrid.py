#!/usr/bin/env python3
"""
Hybrid Strategy: Run two independent sub-strategies.
1) High-PF primary (PF=7, ~1 trade/day) - selective entries
2) Frequency secondary (PF=2+, ~2-4 trades/day) - relaxed entries
Combined: target 3-8 trades/day with PF > 3
"""
...
# ============================================================
# HYBRID BACKTEST: Two strategies with split capital
# ============================================================
def backtest_hybrid(data, primary_cfg, secondary_cfg, primary_pct=0.5):
    """
    Run two strategies simultaneously with split capital.
    primary_pct: fraction of equity allocated to primary strategy
    """
    n = len(data)
    ...
    def make_signals(cfg):
        rt=cfg['rt']; ct=cfg['ct']; tt=cfg['tt']; vsk=cfg.get('vsk',True)
        sf = cfg.get('session_filter','all')
    ...
    p_long, p_short = make_signals(primary_cfg)
    s_long, s_short = make_signals(secondary_cfg)

    eq = INITIAL_EQUITY; curve = np.zeros(n); pnls = []; n_trades = 0
    pos = 0; ep = 0.0; sl_val = 0.0; tp_val = 0.0; sz = 0.0

    # Use primary signals first (higher confidence), then secondary
    for i in range(n):
        if pos != 0:
            if pos == 1:
                if low[i] <= sl_val:
        ...
        if pos == 0:
            signal = 0
            # Try primary first
            if p_long[i]: signal = 1; cfg = primary_cfg
            elif p_short[i]: signal = -1; cfg = primary_cfg
            elif s_long[i]: signal = 1; cfg = secondary_cfg
            elif s_short[i]: signal = -1; cfg = secondary_cfg

            if signal != 0:
                ep = close[i]
                sl_m = cfg['sl']; tp_m = cfg['tp']; risk = cfg['risk']
                if signal == 1:
...
# Primary = high PF
primary = {'rt':0.72,'ct':0.08,'sl':1.5,'tp':3.0,'tt':0.50,'risk':0.01,'vsk':True,'session_filter':'no_asian'}

# Secondaries = more frequent
secondaries = {
    'sec_RT60_CT15_SL10_TP20': {'rt':0.60,'ct':0.15,'sl':1.0,'tp':2.0,'tt':0.50,'risk':0.01,'vsk':True,'session_filter':'all'},
    'sec_RT60_CT15_SL15_TP25': {'rt':0.60,'ct':0.15,'sl':1.5,'tp':2.5,'tt':0.50,'risk':0.01,'vsk':True,'session_filter':'no_asian'},
    'sec_RT65_CT15_SL15_TP25': {'rt':0.65,'ct':0.15,'sl':1.5,'tp':2.5,'tt':0.50,'risk':0.01,'vsk':True,'session_filter':'no_asian'},
    'sec_RT55_CT15_SL12_TP20': {'rt':0.55,'ct':0.15,'sl':1.2,'tp':2.0,'tt':0.50,'risk':0.01,'vsk':True,'session_filter':'no_asian'},
    'sec_RT60_CT20_SL15_TP25': {'rt':0.60,'ct':0.20,'sl':1.5,'tp':2.5,'tt':0.50,'risk':0.01,'vsk':True,'session_filter':'all'},
    'sec_RT60_CT10_SL10_TP25': {'rt':0.60,'ct':0.10,'sl':1.0,'tp':2.5,'tt':0.50,'risk':0.01,'vsk':True,'session_filter':'no_asian'},
    'sec_RT55_CT12_SL15_TP20': {'rt':0.55,'ct':0.12,'sl':1.5,'tp':2.0,'tt':0.50,'risk':0.01,'vsk':True,'session_filter':'all'},
}

print("\n" + "="*70)
print("HYBRID STRATEGY (Primary PF=7 + Secondary)")
print("="*70)
print(f"\n{'#':<4} {'Secondary':<30} {'IS PF':>6} {'T/D':>5} {'OOS PF':>6} {'WR':>5} {'DD%':>5} {'T/D':>5} {'N':>5} {'Ret%':>8}")
print("─"*90)

# Also test primary alone
eq_p, pnl_p, n_p = backtest_hybrid(oos_data, primary, primary)
print(f"{'0':<4} {'PRIMARY ALONE':<30} {'':>6} {'':>5} "
      f"{metrics(eq_p, pnl_p, n_p)['pf']:>6.2f} "
      f"{metrics(eq_p, pnl_p, n_p)['wr']:>4.0%} "
      f"{metrics(eq_p, pnl_p, n_p)['mdd']:>4.1f} "
      f"{metrics(eq_p, pnl_p, n_p)['tpd']:>5.1f} "
      f"{n_p:>5} "
      f"{metrics(eq_p, pnl_p, n_p)['ret']:>+7.1f}%")

results = []
for name, sec in secondaries.items():
    try:
        eq_is, pnl_is, n_is = backtest_hybrid(is_data, primary, sec)
        eq_oos, pnl_oos, n_oos = backtest_hybrid(oos_data, primary, sec)
        m_is = metrics(eq_is, pnl_is, n_is, 550)
        m_oos = metrics(eq_oos, pnl_oos, n_oos, 55)
        if not m_oos or m_oos['n'] < 10: continue
        results.append({'name':name,'sec':sec,'is':m_is,'oos':m_oos})

        in_range = "  ✅" if 2 <= m_oos['tpd'] <= 10 else ""
        print(f"{len(results):<4} {name:<30} {m_is['pf']:>6.2f} {m_is['tpd']:>5.1f} "
...
# Top by PF in range
print("\n" + "="*70)
print("TOP HYBRID CONFIGS IN RANGE (2-10 trades/day)")
print("="*70)
in_range = [r for r in results if 2 <= r['oos']['tpd'] <= 10 and r['oos']['pf'] >= 2.5]
in_range.sort(key=lambda x: x['oos']['pf'], reverse=True)

for i, r in enumerate(in_range[:5]):
    oos = r['oos']
    print(f"  {i+1}. {r['name']:<30} PF={oos['pf']:.2f} WR={oos['wr']:.0%} DD={oos['mdd']:.1f}% "
          f"T/D={oos['tpd']:.1f} N={oos['n']} Ret={oos['ret']:+.1f}%")

# Best by return in range
print("\n" + "="*70)
print("TOP BY RETURN (2-10 trades/day)")
print("="*70)
by_ret = sorted(in_range, key=lambda x: x['oos']['ret'], reverse=True)
for i, r in enumerate(by_ret[:5]):
    oos = r['oos']
    print(f"  {i+1}. {r['name']:<30} Ret={oos['ret']:+.1f}% PF={oos['pf']:.2f} "
          f"T/D={oos['tpd']:.1f} DD={oos['mdd']:.1f}%")

# Save best
if in_range:
    best = in_range[0]
    with open('best_hybrid.json', 'w') as f:
        json.dump({'primary': primary, 'secondary': best['sec'], 'oos': best['oos'], 'is': best['is']},
                  f, indent=2, default=str)
    print(f"\nSaved best_hybrid.json")

print("\n✅ Hybrid experiments complete!")
